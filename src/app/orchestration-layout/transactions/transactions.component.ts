...
export class TransactionsComponent implements OnInit, AfterContentInit {
  ...
  pageIndex: number = 1;
  pageSize: number = 10;
  @ViewChild('advanceSearchDialog')
  advanceSearchDialog!: TemplateRef<any>;
  private advanceSearchDialogRef!: MatDialogRef<TemplateRef<any>>;
  cloudProvideList:any=[];

  // @ViewChild('advanceSearchDialog', {static: false}) advanceSearchDialog: TemplateRef<any> | undefined;
  timeout: any;
  ...
  ngOnInit(): void {
    ...
    this.status =
      this.route.snapshot.queryParams.get('status') || '';
  ...
  applyFilter(filterValue: any) {
    ...
    this.transactionsData.filter = filterString.trim().toLowerCase();
  }
  advanceSearch():void{
    // eslint-disable-next-line security/detect-non-literal-fs-filename
    this.advanceSearchDialogRef = this.dialog.open(this.advanceSearchDialog,{ width: '35%',height:'95%',position: {right:'0px',bottom:'14px'}});
    this.advanceSearchDialogRef.afterClosed().subscribe(result => {
